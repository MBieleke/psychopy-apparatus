...
def parse_force_data_payload(payload: bytes) -> dict:
    ...
    Payload format:
    - legacy: time_us(u32) + value(i16, 0.1N units) + device(u8)
    - experimental3: time_us(u32) + value(i16, 0.1N units) + adc_raw_counts(i16) + device(u8)
    ...
    -------
    dict
        Dictionary with keys: 'time_us', 'value', 'raw_value', 'device', 'adc_raw_counts'
    ...
    """
    if len(payload) == 7:
        time_us, raw_value, device = struct.unpack('<IhB', payload)
        adc_raw_counts = None
    elif len(payload) == 9:
        time_us, raw_value, adc_raw_counts, device = struct.unpack('<IhhB', payload)
    else:
