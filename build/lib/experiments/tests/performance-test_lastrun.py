...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Maik Bieleke\\My Drive\\Labor\\psychopy-apparatus\\experiments\\tests\\performance-test_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        countdown_led.setLights("all", 'black')
        ...
        while continueRoutine and routineTimer.getTime() < 30.0:
            ...
            if countdown_led.status == STARTED:
                ...
                if tThisFlipGlobal > countdown_led.tStartRefresh + 30-frameTolerance:
                    ...
                    if True:
                        countdown_led.turnOffLights("all")
            # Run 'Each Frame' code from countdown_code
            if t - led_last_update >= led_update_interval:
                holes = list(range(num_holes))
                colors = [color_list[(led_step + i) % len(color_list)] for i in range(num_holes)]
                countdown_led.setLights(holes, colors)
            ...
            if apparatusForce_2.status == NOT_STARTED and t >= 0-frameTolerance:
                ...
                apparatusForce_2.startForceMeasurement(100, 'both')
        ...
        trials.addData('apparatusForce_2.dynamometer', 'both')
        ...
        if True:
            ...
            _records = apparatusForce_2.forceRows if hasattr(apparatusForce_2, 'forceRows') else []
        ...
        trials.addData('apparatusReed.reedCurrentStates', apparatusReed.reedCurrentStates)
        trials.addData('apparatusReed.reedActiveHoles', apparatusReed.reedActiveHoles)
        trials.addData('apparatusReed.reedNewInsertions', apparatusReed.reedNewInsertions)
        trials.addData('apparatusReed.reedNewRemovals', apparatusReed.reedNewRemovals)
        trials.addData('apparatusReed.reedLatestEvent', apparatusReed.reedLatestEvent)
        trials.addData('apparatusReed.reedFrameTimes', apparatusReed.reedFrameTimes)
        trials.addData('apparatusReed.reedFrameStates', apparatusReed.reedFrameStates)
        trials.addData('apparatusReed.reedFrameActiveHoles', apparatusReed.reedFrameActiveHoles)
